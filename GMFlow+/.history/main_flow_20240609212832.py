...
def get_args_parser():
    ...
    # evaluation
    parser.add_argument(
        "--eval", action="store_true", help="evaluation after training done"
    )
    ...
    parser.add_argument(
        "--task", default="flow", choices=["flow", "stereo", "depth"], type=str
    )
    parser.add_argument(
        "--num_scales", default=1, type=int, help="feature scales: 1/8 or 1/8 + 1/4"
    )
    ...
    parser.add_argument(
        "--attn_type", default="swin", type=str, help="attention function"
    )
    parser.add_argument(
        "--attn_splits_list",
        default=[2],
    ...
    # loss
    parser.add_argument(
        "--gamma", default=0.9, type=float, help="exponential weighting"
    )
    ...
    parser.add_argument(
        "--pred_bidir_flow", action="store_true", help="predict bidirectional flow"
    )
    parser.add_argument(
        "--pred_bwd_flow", action="store_true", help="predict backward flow only"
    )
    parser.add_argument(
        "--fwd_bwd_check",
        action="store_true",
    ...
    parser.add_argument(
        "--launcher", default="none", type=str, choices=["none", "pytorch"]
    )
    parser.add_argument("--gpu_ids", default=0, type=int, nargs="+")

    # misc
    parser.add_argument(
        "--count_time", action="store_true", help="measure the inference time"
    )
...
def main(args):
    print_info = (
        not args.eval
        and not args.submission
        and args.inference_dir is None
        and args.inference_video is None
    )
    ...
    if args.resume:
        ...
        model_without_ddp.load_state_dict(
            checkpoint["model"], strict=args.strict_resume
        )
    ...
    if args.eval:
        ...
        if args.save_eval_to_file:
            ...
            with open(val_file, "a") as f:
                ...
                eval_metrics = [
                    metric for metric in metrics if metric in val_results.keys()
                ]
    ...
    if args.local_rank == 0:
        ...
        logger = Logger(
            lr_scheduler, summary_writer, args.summary_freq, start_step=start_step
        )
            ...
            if total_steps % args.save_ckpt_freq == 0 or total_steps == args.num_steps:
                if args.local_rank == 0:
                    ...
                    checkpoint_path = os.path.join(
                        args.checkpoint_dir, "step_%06d.pth" % total_steps
                    )
                    torch.save(
                        {"model": model_without_ddp.state_dict()}, checkpoint_path
                    )

            if total_steps % args.save_latest_ckpt_freq == 0:
                checkpoint_path = os.path.join(
                    args.checkpoint_dir, "checkpoint_latest.pth"
                )
            ...
            if total_steps % args.val_freq == 0:
                ...
                if "tub" in args.val_dataset:
                    results_dict = validate_tub(
                        ...
                        prop_radius_list=args.prop_radius_list,
                        num_reg_refine=args.num_reg_refine,
                        tub_IM=5 if args.tub_IM is "all_train" else 4,
                        root=args.tub_root,
                        ie_eval=args.eval,
                    )
                    if args.local_rank == 0:
                ...
                if args.local_rank == 0:
                    ...
                    with open(val_file, "a") as f:
                        ...
                        metrics_values = [
                            val_results[metric] for metric in eval_metrics
                        ]
                        ...
                        if args.evaluate_matched_unmatched:
                            ...
                            f.write(
                                ("| {:25.3f} " * num_metrics).format(*metrics_values)
                            )
                        else:
                            f.write(
                                ("| {:>20} " * num_metrics + "\n").format(*eval_metrics)
                            ...
                            f.write(
                                ("| {:20.3f} " * num_metrics).format(*metrics_values)
                            )
