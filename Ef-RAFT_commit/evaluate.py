...
def angular_error(flow_pr, flow_gt):
    ...
    dot_product = torch.sum(flow_pr_norm * flow_gt_norm, dim=0)
    angle_error = torch.acos(torch.clamp(dot_product, -1.0, 1.0))
    return angle_error
...
def warp(image, flow):
    ...
    # Sample image with new grid
    output = F.grid_sample(image, new_grid, mode='bilinear', padding_mode='border')
    return output
